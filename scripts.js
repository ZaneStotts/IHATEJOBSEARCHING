...
// APPLICATION COUNTER FEATURE

// This function counts the number of applications and displays them
function countTotalApplications() {

    // This constant variable represents all elements with the ".application" class
    const applications = document.querySelectorAll(".application");

    // This constant variable represents the applications total
    const applicationsTotal = applications.length;

    // This constant variable decides where the total number should be displayed in the HTML
    const output = document.querySelector(".job-number-container p");

    // If the element (".job-number-container p") exists , then update the text content to display the total amount of applications
    if (output) {
        output.textContent = applicationsTotal;
    }

    // Returns the total amount of applications in case it needs to be used in the future
    return applicationsTotal;
}

// This group of lines makes sure the HTML is loaded before running the previously described function
